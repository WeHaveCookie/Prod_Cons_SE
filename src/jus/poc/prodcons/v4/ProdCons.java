...
public class ProdCons implements Tampon {
	...
	synchronized public Message get(_Consommateur arg0) throws Exception,InterruptedException {
		...
		if(m.IsConso()){
			...
			if (impression == 1){
				System.out.println("Consommateur : " + arg0.identification() + " veux retirer " + m + " - NbExemplaireConso " + m.getNbConso() + " - NbEx : " + m.getNbMsgDepos());
			}
			out = (out + 1) % taille();
			nbCasePleine--;
			...
			if (impression == 1){
				System.out.println("Consommateur_Retrait : " + arg0.identification() + " retire " + m + " - NbExemplaireConso " + m.getNbConso() + " - NbEx : " + m.getNbMsgDepos());
			}
		}
		mutexC.v();
		FileCons.v();
